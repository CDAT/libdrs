...
c
     ...
     *          getedim,setdim,setvdim,putdat
      integer*4 cw_aslun,cw_cllun,cw_setname,cw_getname,cw_getdat,cw_putvdim,
     *          cw_cluvdb,cw_getedim,cw_setdim,cw_setvdim,cw_putdat
c
...
c     
      ...
      write(6,*) filein
c
      ierr=cw_aslun(7,filein,8,' ',IDRS_READ,55,1)
c
...
c
      ...
      write(6,*) filein
c
      ierr=cw_aslun(9,filein,10,' ',IDRS_READ)
c
...
c
c         ierr=cw_cluvdb()
c         ierr=cw_setname(' ','latitude',' ',' ',' ')
c         ierr=cw_getdat(7,gausslat,idrs_bytes_per_word*nlat)
c         ierr=cw_getname(vsource2d,vname2d,vtitle2d,vun2d,
...
c
C      ierr=cw_cluvdb()
C      ierr=cw_setname(' ','plev',' ',' ',' ')
C      ierr=cw_getdat(7,plevel,idrs_bytes_per_word*n3)
C      ierr=cw_getname(vsource3d,vname3d,vtitle3d,vun3d,
...
c
         ierr=cw_cluvdb()
         ierr=cw_setname(' ',vname(1),' ',' ',' ')
         ierr=cw_inqdict(7,IDRS_GETFIRSTVAR)
         ierr=cw_getedim(2,vsource2d,vname2d,vtitle2d,
     *        vun2d,ntype2d,numdim2,xt1,xtn)
         ierr=cw_getcdim(2,vsource2d,vname2d,vtitle2d,
...
c     
c        ierr=cw_setdim(ndim,'dimname',' ',num,start,finish)
c   
         ierr=cw_setdim(1,'longitude',' ',nlon,xlonmin,xlonmax)
c         ierr=cw_setvdim(2,' ','latitude',' ',' ',-45.4203,45.4203)
c         ierr=cw_setvdim(3,' ','plev',' ',' ',xlevel,xlevel)
         ierr=cw_setdim(2,vname2d,vun2d,1,times(itot),times(itot))
c
c
         ierr=cw_getdat(7,din,IDRS_BYTES_PER_WORD*nlon)
c
c         ierr=cw_getname(vsourcein1,vname(1),vtitle(1),vun(1),
c     *        vdate(1),vtime(1),vtype(1),numdim(1))
c
c     EXTRACT LONGITUDE INFORMATION
c
c         ierr=cw_getedim(1,vsource1d,vname1d,vtitle1d,
c     *        vun1d,ntype1d,nlong,xlong1,xlongn)
c         ierr=cw_getcdim(1,vsource1d,vname1d,vtitle1d,
c     *        vun1d,ntype1d,nlong,along,longn)
c
c         if(nspace .eq. 0)then
c            ierr=cw_getedim(2,vsource2d,vname2d,vtitle2d,
c     *           vun2d,ntype2d,nlatt,xlat1,xlatn)
c            go to 1013
c
c            ierr=cw_getcdim(2,vsource2d,vname2d,vtitle2d,
c     *           vun2d,ntype2d,nlat,gausslat,latn)
c         endif
c
c        ierr=cw_getedim(3,vsourcetd,vnametd,vtitletd,
c     *        vuntd,ntypetd,numdimtd,xn31,xn3n)
c
c************************************************************************
c        print *,'Reading ',vname(2)
         ierr=cw_cluvdb()
         ierr=cw_setname(' ',vname(2),' ',' ',' ')
c
c     ONLY USE SETDIM IF YOU WANT TO EXTRACT A PORTION OF VNAME()
c     where
c     ndim    = which x, y, z, t coordinate (integer 1-4) (e.g. 3)
...
c         ierr=cw_setvdim(3,' ','plev',' ',' ',xlevel,xlevel)
         ierr=cw_setdim(2,vname2d,vun2d,1,times(itot),times(itot))
c
         ierr=cw_getdat(9,ddep,IDRS_BYTES_PER_WORD*nlon)
c
c         ierr=cw_getname(vsourcein2,vname(2),vtitle(2),vun(2),
...
c
         ...
         ierr=cw_setname(' ',vname(1),' ',' ',' ')   
         ierr=cw_setdim(1,'longitude',' ',nlon,xlonmin,xlonmax)
         ierr=cw_setdim(2,vname2d,vun2d,1,times(iold),times(iold))
         ierr=cw_getdat(7,din,IDRS_BYTES_PER_WORD*nlon)
         do ilon=1,nlon
...
c
         ...
         ierr=cw_setname(' ',vname(1),' ',' ',' ')   
         ierr=cw_setdim(1,'longitude',' ',nlon,xlonmin,xlonmax)
         ierr=cw_setdim(2,vname2d,vun2d,1,times(inew),times(inew))
         ierr=cw_getdat(7,din,IDRS_BYTES_PER_WORD*nlon)
         do ilon=1,nlon
...
c
         ...
         enddo
c
         call lsfit_moving(nlon*ntot,x8_old,x8_new,y8_old,y8_new,nlon,slope1,yint1,F,R)
