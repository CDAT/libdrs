...
c  2011: changed cw_ to fcw_, and other changes to work well on 64-bit Macintosh OS X
c
c  to compile and run on a Macintosh, for debugging:
c gfortran -g3 -save-temps -O0 -arch x86_64 -L/usr/local/lib -L${HOME}/cdat/libcdms/lib -lcdms -lnetcdf -lz -ldrs -lc -lm -o  drs_pat_cor_2d_hov_fcw  drs_pat_cor_2d_hov_fcw.f
c dsymutil drs_pat_cor_2d_hov_fcw
c drs_pat_cor_2d_hov_fcw    or use the script
c drs_pat_cor_2d_hov_fcw_run
c
...
c     Why real*4 in the following line:  The C DRS wrappers are not good at doing conversions
c     between float and double (real*8).  Anyway, Most data in files is float (real*4).
...
c     The next line is useful only when running under Valgrind to check for memory errors;
c     it prevents a memory error in the test  (din(1) .lt. 9.9 e19)...
      din(1) = 1.0e20
